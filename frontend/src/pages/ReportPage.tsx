...
function formatSourceText(value: string): string {
  return escapeHtml(value).replace(/\n/g, '<br />');
}

function buildSourceDocumentsMarkup(content: ReportContent | null, resumePdfObjectUrl?: string): string {
  const sourceDocuments = content?.source_documents;
  const resumeText = sourceDocuments?.resume_text?.trim();
  const postings = (sourceDocuments?.postings || []).map((posting) => posting.trim()).filter(Boolean);

  if (!resumeText && !postings.length && !resumePdfObjectUrl) {
    return '';
  }

  const resumePdfMarkup = resumePdfObjectUrl
    ? `<section style="margin-top:28px;padding-top:20px;border-top:1px solid #e1d6c3;">
        <h2 style="margin:0 0 12px;font-size:13px;text-transform:uppercase;letter-spacing:0.08em;color:#5b6662;">Uploaded Resume PDF</h2>
        <div style="padding:12px;border-radius:14px;border:1px solid #e1d6c3;background:#fbf8f2;">
          <object data="${resumePdfObjectUrl}" type="application/pdf" style="width:100%;height:860px;border-radius:10px;">
            <p style="margin:0;font-size:13px;color:#2d3834;">Your browser cannot embed this PDF preview. Use the extracted resume text section below.</p>
          </object>
        </div>
      </section>`
    : '';
  ...
  const resumeMarkup = resumeText
    ? `<section style="margin-top:28px;padding-top:20px;border-top:1px solid #e1d6c3;">
        <h2 style="margin:0 0 12px;font-size:13px;text-transform:uppercase;letter-spacing:0.08em;color:#5b6662;">Uploaded Resume Text</h2>
        <div style="padding:16px;border-radius:14px;border:1px solid #e1d6c3;background:#fbf8f2;font-size:13px;line-height:1.7;color:#2d3834;">${formatSourceText(resumeText)}</div>
      </section>`
    : '';

  const postingsMarkup = postings.length
    ? `<section style="margin-top:28px;padding-top:20px;border-top:1px solid #e1d6c3;">
        <h2 style="margin:0 0 12px;font-size:13px;text-transform:uppercase;letter-spacing:0.08em;color:#5b6662;">Uploaded Job Postings</h2>
        <div style="display:grid;gap:12px;">
          ${postings
            .map(
              (posting, index) => `<article style="padding:16px;border-radius:14px;border:1px solid #e1d6c3;background:#fbf8f2;">
                <p style="margin:0 0 10px;font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#5b6662;">Posting ${index + 1}</p>
                <div style="font-size:13px;line-height:1.7;color:#2d3834;">${formatSourceText(posting)}</div>
              </article>`,
            )
            .join('')}
        </div>
  ...
  return `${resumePdfMarkup}${resumeMarkup}${postingsMarkup}`;
}
...
export function buildPreviewHtml(content: ReportContent | null, resumePdfObjectUrl?: string): string {
  if (!content) {
  ...
  const sourceDocumentsMarkup = buildSourceDocumentsMarkup(content, resumePdfObjectUrl);
        ...
        <section className="report-meta">
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                const response = await authenticatedApiFetch(`/report/${encodeURIComponent(reportContent.job_id)}/resume-file`);
                if (response.ok) {
                ...
                // Keep PDF export functional even when resume-file fetch fails.
              }

              const html = buildPreviewHtml(reportContent, resumePdfObjectUrl);
